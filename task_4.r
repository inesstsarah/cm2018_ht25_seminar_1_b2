...
ggplot(counts, aes(x = factor(treatment), y = n, fill = response)) +
  ...
  scale_x_discrete(labels = c("Control", "Low", "Medium", "High"))+
  labs(
    x = "Dose",
    y = "Proportion",
    fill = "SPMSQ Category",
...
ggplot(merged_data, aes(x = response, y = NFL, fill = response)) +
  ...
  geom_text(data = box_counts,
            aes(x = response, y = max(merged_data$NFL, na.rm = TRUE) * 1.4, 
                label = paste0("n=", n)),
            inherit.aes = FALSE,
            vjust = 0,
            size = 3.5) +
  labs(
    title = "NFL levels by SPMSQ category",
    x = "SPMSQ Category",
    y = "NFL concentration"
  ) +
  theme_minimal() +
...
# Alluvial plot_____________________________________________________________________________
...
# Paired staple plot_________________________________________________________________________
df_t4b %>%
  group_by(OCC, response) %>%
  summarise(n = n(), .groups = "drop") %>%
...
# Computing how many patients 
# got better, worse or same___________________________________________________________________

# Sorting so that each patient gets its own row 
df_sorted <- df_sorted %>%
  mutate(
    OCC1 = factor(OCC1, levels = c("Normal", "Mild", "Moderate", "Severe"), ordered = TRUE),
    OCC2 = factor(OCC2, levels = c("Normal", "Mild", "Moderate", "Severe"), ordered = TRUE),
    change = case_when(
      OCC2 > OCC1 ~ "Worse",
      OCC2 < OCC1 ~ "Better",
      TRUE ~ "Same"
...
# Bar plot___________________________________________________________________________________
change_counts <- df_sorted %>%
  count(change)
